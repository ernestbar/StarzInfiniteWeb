...
namespace StarzInfiniteWeb
{
    public partial class vuelos : System.Web.UI.Page
    {
        ...
        protected void btnComprar_Click(object sender, EventArgs e)
        {
                        ...
                        if (DT_dom.Rows.Count > 0)
                        {
                            foreach (DataRow dr in DT_dom.Rows)
                            {
                                ...
                                { 
                                    ...
                                    lblAreolineaNombResIdaRes.Text = dr["descripcion"].ToString() + " (" + datos[5] + ")"; 
                                }
                            }
                        }
                        if (ida == 1)
            ...
            else
            {
                if (checkIda > 0 && checkVuelta > 0)
                {
                    ...
                    Repeater10.DataBind();
                    Repeater12.DataSource = dt_vuelta;
                    Repeater12.DataBind();
                    DataTable DT_dom = new DataTable();
                    DT_dom = Dominios.Lista("AEROLINEA");
                    if (DT_dom.Rows.Count > 0)
                    {
                        foreach (DataRow dr in DT_dom.Rows)
                        {
                            if (dr["codigo"].ToString() == datos[5])
                            {
                                ...
                                lblAreolineaNombResVueltaRes.Text = dr["descripcion"].ToString() + " (" + datos[5] + ")";
                            }
        ...
        protected void btnEnviarCorreo_Click(object sender, EventArgs e)
        {
            ...
            string resp_email = objC.enviar(txtEmailTit.Text, "Confirmación de reserva StarzInfinite", hfDiv.Value, "");
            if (resp_email == "OK")
                lblAviso.Text = "Correo enviado correctamente a: " + txtEmailTit.Text;
            else
